...
public class TaskManagerBot extends TelegramLongPollingBot {
        ...
        try {
            if (text.length() > 6) {
                ...
                sendMessage(chatId, "👋 Привет! Я бот TaskManager.\n\n" +
                    ...
                    "1️⃣ Отсканируйте QR-код на сайте\n" +
                    "2️⃣ Или используйте команду /link your@email.com\n\n" +
            ...
            sendMessage(chatId, "❌ Ошибка при обработке команды: " + e.getMessage());
        }
    }
    ...
    private void handleNewTask(Long chatId, String text) {
        try {
            ...
            if (parts.length < 5) {
                sendMessage(chatId, "Используйте формат:\n/new Название | Описание | Категория | Приоритет | Дедлайн");
                return;
            }
